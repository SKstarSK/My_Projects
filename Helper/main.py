        ...
        for filename in os.listdir(folder_track):
            ...
            if len(extension) > 1 and ("jpg" in extension or "svg" in extension or "png" in extension):
                ...
                if filename not in os.listdir(folder_track + "/" + myWays[4]):
                    ...
                    os.remove(folder_track + "/" + filename)
            elif len(extension) > 1 and ("doc" in extension or "docx" in extension or "pdf" in extension or "ppt" in extension or "pptx" in extension or "xlsx" in extension):
                if filename not in os.listdir(folder_track + "/" + myWays[5]):
                    handle.replace_file(folder_track, filename, myWays[5])
                else:
                    os.remove(folder_track + "/" + filename)
            elif len(extension) > 1 and (os.path.isfile(folder_track+"/"+filename)) or (os.path.isdir(folder_track+"/"+filename)):
...
folder_track = '/Users/bolot/Downloads'
